...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 25 17:12:10 2019
...
def o_model_ver1(T,N,tnum,xnum,hic,hbc,uic,ubc):
  
    deltx=N/xnum
    deltt=T/tnum
    
    p2=((2/3)*deltx+ubc[1]*deltt)/(uic[1]-ubc[1])
    p1=deltt*uic[1]+deltt*p2
    p3=2*hic[0]-uic[0]/math.sqrt(2*10)
    ...
    for i in range(tnum):
        ...
        for j in range(xnum):
            if j==0:
                ...
                ut=((2/3)*(j*deltx)+p1)/((i*deltt)+p2)
                utm.append(ut)
                htm.append(0.5*(ut/math.sqrt(2*10)+p3))
        u.append(utm)
        h.append(htm)
...
if __name__=='__main__':
    ...
    u,h,p1,p2,p3=o_model_ver1(T,N,tnum,xnum,hic,hbc,uic,ubc)
    #plt.plot(u)
    #plt.plot(h)
    ...
    steps=1000
    u_K=np.array(Koopman(t,u,steps))
    print(u_K.shape)

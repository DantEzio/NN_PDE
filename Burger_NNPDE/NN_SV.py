...
class NN_SV:
    ...
    def _build_model(self):
        ...
        #build encoding and decoding model
        #encoding-decoding
        self.W1e=tf.Variable(tf.truncated_normal([self.xnum,self.NN_en_n],stddev=0.1))
        self.b1e=tf.Variable(tf.zeros([self.NN_en_n]))
        
        self.W2e=tf.Variable(tf.truncated_normal([self.NN_en_n,self.NN_en_n],stddev=0.1))
        self.b2e=tf.Variable(tf.zeros([self.NN_en_n]))
    
        self.W1d=tf.Variable(tf.truncated_normal([self.NN_de_n,self.NN_de_n],stddev=0.1))
        self.b1d=tf.Variable(tf.zeros([self.NN_de_n]))
        
        self.W2d=tf.Variable(tf.truncated_normal([self.NN_de_n,self.xnum],stddev=0.1))
        self.b2d=tf.Variable(tf.zeros([self.xnum]))
            

        #K近似
        self.W1=tf.Variable(tf.truncated_normal([self.NN_de_n,self.NN_de_n],stddev=0.1))
        self.b1=tf.Variable(tf.zeros([self.NN_de_n]))
        self.W2=tf.Variable(tf.truncated_normal([self.NN_de_n,self.NN_de_n],stddev=0.1))
        self.b2=tf.Variable(tf.zeros([self.NN_de_n]))
    
    #EDMD
    def EDMD(self):
        self.h1=tf.nn.tanh(tf.matmul(self.x_,self.W1e)+self.b1e)
        self.hmid=tf.nn.tanh(tf.matmul(self.h1,self.W2e)+self.b2e)
        self.h2=tf.nn.tanh(tf.matmul(self.hmid,self.W1d)+self.b1d)
        self.hd=tf.nn.tanh(tf.matmul(self.h2,self.W2d)+self.b2d)
        
        self.ht1=tf.nn.tanh(tf.matmul(self.hmid,self.W1)+self.b1)
        self.ht2=tf.nn.tanh(tf.matmul(self.ht1,self.W2)+self.b2)
        
        self.htout1=tf.nn.tanh(tf.matmul(self.ht2,self.W1d)+self.b1d)
        self.htout2=tf.nn.tanh(tf.matmul(self.htout1,self.W2d)+self.b2d)#最终输出
        
        self.loss1=tf.sqrt(tf.reduce_mean(tf.square(self.hd - self.y_ed)))
        self.loss2=tf.sqrt(tf.reduce_mean(tf.square(self.htout2 - self.y_pre)))
        
        self.train1=tf.train.AdamOptimizer(0.001).minimize(self.loss1+self.loss2)
    ...
    def training(self):
        ...
        for j in range(self.steps):
            for i in range(D1.shape[0]-1):
                ...
                self.sess.run(self.train1,feed_dict={self.x_:bx,self.y_ed:byed,self.y_pre:bypre})
                
        output1,output2=[],[]
        
        for i in range(D1.shape[0]):
            batch_xs=np.array([D1[0],D2[0]])
            batch_xs=batch_xs.reshape(1,batch_xs.shape[0],batch_xs.shape[1])
            pre=self.sess.run(self.htout2,feed_dict={self.x_:batch_xs})      
            result=pre#((pre+1)/2)*(outmax[-1]-outmin[-1])+outmin[-1]
            #batch_xs=result
            #batch_xs[0][0][0]=D1[i][0]
            #batch_xs[0][1][0]=D2[i][0]
            #print(result.shape)
